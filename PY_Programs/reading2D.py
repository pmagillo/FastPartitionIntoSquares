...
def readImageFormat(filename, verbose=False):
  ...
  if INVERT: select = lambda x,y : img[x][y]==0
  else:      select = lambda x,y : img[x][y]>0
  for x in range(width):
    for y in range(height):
      if select(x,y): 
...
"""
This main is only to check that we can read images correctly.
It simply reads the image and prints its dimensions.
"""
